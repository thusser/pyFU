...
from pyFU.utils    import hist_rebin, read_tables, convolve_rebin_table, parse_arguments, \
			write_tables, read_spectrum, get_infiles_and_outfiles, initialize_logging
...
def main () :
	...
	"""
	arguments = {
		'clobber':    {'path':None,'default':False, \
						'flg':'-c','type':bool,'help':'permit overwriting of tables'},
		'dispersion': {'path':'rebin:','default':None, \
						'flg':'-d','type':float,'help':'output dispersion in nm/pixel'},
		'errcol':     {'path':'rebin:','default':'err_flux', \
						'flg':'-E','type':str,'help':'name of flux error column)'},
		'flxcol':     {'path':'rebin:','default':'flux', \
						'flg':'-F','type':str,'help':'name of flux column)'},
		'generic': {'path':None,'default':None, \
						'flg':'-G','type':str,'help':'YAML file for generic rebin image configuration info'},
		'infiles':    {'path':'rebin:','default':None, \
						'flg':'-i','type':str,'help':'input FITS binary table(s) containing spectra'},
		'limits':     {'path':'rebin:','default':None, \
						'flg':'-l','type':str,'help':'wavelength limits w1,w2 [nm] of output spectrum'},
		'npixels':    {'path':'rebin:','default':None, \
						'flg':'-n','type':int,'help':'number of output pixels'},
		'outfiles':   {'path':'rebin:','default':None, \
						'flg':'-o','type':str,'help':'output FITS table(s)'},
		'plot':       {'path':None,'default':False, \
						'flg':'-p','type':bool,'help':'plot result'},
		'pixcol':     {'path':'rebin:','default':'pixel', \
						'flg':'-P','type':str,'help':'name of pixel column)'},
		'title':      {'path':None,'default':'rebin', \
						'flg':'-T','type':str,'help':'logging title)'},
		'reference':  {'path':'rebin:','default':None, \
						'flg':'-R','type':str,'help':'pathname of reference spectrum file'},
		'wavcol':     {'path':'rebin:','default':'wavelength', \
						'flg':'-w','type':str,'help':'name of wavelength column'},
		'wcol':       {'path':'rebin:','default':'wavelength', \
						'flg':'-W','type':str,'help':'name of wavelength column in reference'},
		'wresolution':{'path':'rebin:','default':None, \
						'flg':'-r','type':float,'help':'wavelength resolution to apply'},
		'yaml':       {'path':'rebin:','default':None, \
	...
	for infile,outfile in zip(infiles,outfiles) :
		...
		if len(spectra) == 0 :
			...
			if ('limits' not in info) or (info['limits'] is None) :
				logging.info ('using wavelength bounds of #1/{0} spectrum'.format(len(spectra)))
				spectrum = spectra[0]
			...
			elif ('npixels' not in info or info['npixels'] is None or info['npixels'] <= 0) and ('dispersion' in info and info['dispersion'] is not None) :
				info['npixels'] = (wave2-wave1)/info['dispersion']+1

			# ... GET DISPERSION FROM NUMBER OF PIXELS AND LIMITS
			elif ('dispersion' not in info or info['dispersion'] is None or info['dispersion'] <= 0.) and ('npixels' in info and info['npixels'] is not None) :
				info['dispersion'] = (wave2-wave1)/(info['npixels']-1)

			# ... HAVE DISPERSION AND NPIXELS, SO USE STARTING WAVELENGTH
			elif ('dispersion' in info and info['dispersion'] is not None and info['dispersion'] > 0.) and \
				 ('npixels'    in info and info['npixels']    is not None and info['npixels']    > 0 ) :
				wave2 = wave1+(info['npixels']-1)*info['dispersion']
				logging.warning ('using given npixels, dispersion and resulting ending wavelength {0:.2f}!'.format(wave2))
		...
		if 'wresolution' in info and info['wresolution'] is not None :
			logging.info ('degrading input spectra to resolution {0:.3f} nm ...'.format(info['wresolution']))
		else :
		...
		for i in range(len(spectra)) :
			...
			if args.plot :
				...
				if ('errcol' not in info) or (info['errcol'] is None) :
					plt.plot (spectrum[info['wavcol']],spectrum[info['flxcol']],'-',label='original',drawstyle='steps-mid')
					plt.plot (wave,flux,'.',label='rebinned',markersize=4,color='black')
				else :
					err  = tab[info['errcol']]
					plt.plot (spectrum[info['wavcol']],spectrum[info['flxcol']],'-',label='original',drawstyle='steps-mid')
					plt.errorbar (wave,flux,yerr=err,fmt='.',label='rebinned',markersize=4,color='black')
