...
def help():
    ...
    print(
        "\tbi{as}     {name of masterbias or bias images directory} {optional name of masterbias}"
    )
    print(
        "\tca{lib}    {name of raw image file}                      {name of calibrated image file}"
    )
    print("\tcr{eate}   {name of fake image to create using configuration file}")
    print("\tco{nfig}   {name of pyfu YAML configuration file}")
    print(
        "\tda{rk}     {name of unitdark or dark images directory}   {optional path of unitdark}"
    )
    print(
        "\tex{tract}  {name of image file}                          {optional path of extracted table}"
    )
    print("\tfh{eader}  {name of FITS file}")
    print(
        "\tfl{at}     {name of masterflator flat images directory}  {optional path of masterbias}"
    )
    print("\the{lp}")
    print("\tid{isplay} {name of image file to display}")
    print("\tlo{gging}  {debug,info}")
    print(
        "\tpi{mage}   {pathname of table to be imaged}              {optional path of PNG image integrated in pixels}"
    )
    print(
        "\tpl{ot}     {name of table file}                          {index of spectrum}"
    )
    ...
    print("\tst{atus}")
    print(
        "\ttr{ace}    {pathname of image to be traced}              {optional path of trace configuration file}"
    )
    print(
        "\twi{mage}   {pathname of table to be imaged}              {optional path of PNG image integrated in wavelength}"
    )
    ...
    try:
        while ok:
            ...
            if cmd.startswith("q"):  # QUIT
                ...
                logging.info(
                    'Creating the generic configuration YAML file "generic.yaml" ...'
                )
                ...
                if infile == "none":  # a) USED TO AVOID BIAS CORRECTION, or
                    ...
                    infile
                ):  # b) FITS-FILE -> SET masterbias TO THIS FILE, or
                ...
                cmd.startswith("id") or cmd.startswith("di")
            ) and nparts > 1:  # IMAGE DISPLAY
                ifu_cmd = (
                    f'ifudis -i "{fullpath(parts[1])}" -l'  # DEFAULT IS TO PLOT TRACES
                )
                do_cmd(ifu_cmd + extn)
                ...
                if infile == "none":  # a) USED TO AVOID FLAT CORRECTION, or
                    ...
                    infile
                ):  # b) FITS-FILE -> SET masterflat TO THIS FILE, or
                ...
                nparts == 5 and parts[1] == "="
            ):  # MATHS : outfile = infile1 {operation} infile2
