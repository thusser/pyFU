...
from pyFU.utils    import hist_rebin, read_spectrum, parse_arguments, convolve_rebin, initialize_logging
...
def main () :
	...
	"""
	arguments = {
		'atlas':      {'path':'solar:','default':None, \
						'flg':'-a','type':str,'help':'input solar reference atlas (FITS table)'},
		'dispersion': {'path':'solar:','default':None, \
						'flg':'-d','type':float,'help':'output dispersion in nm/pixel'},
		'fcol':       {'path':'solar:','default':'flux', \
						'flg':'-f','type':str,'help':'name of input flux table column)'},
		'flxcol':     {'path':'solar:','default':'flux', \
						'flg':'-F','type':str,'help':'name of output flux table column)'},
		'generic': {'path':None,'default':None, \
						'flg':'-G','type':str,'help':'YAML file for generic solar spectrum configuration info'},
		'limits':     {'path':'solar:','default':None,'dshow':'all', \
						'flg':'-l','type':str,'help':'wavelength limits w1,w2 [nm] of output spectrum'},
		'npixels':    {'path':'solar:','default':None, \
						'flg':'-n','type':int,'help':'number of output pixels'},
		'outfile':    {'path':'solar:','default':None, \
						'flg':'-o','type':str,'help':'output FITS table'},
		'plot':       {'path':None,'default':False, \
						'flg':'-p','type':bool, 'help':'plot result'},
		'resolution': {'path':'solar:','default':None,'dshow':'<wave>/(3pix*dispersion)', \
						'flg':'-R','type':float,'help':'spectral resolution R=lambda/dlambda'},
		'wcol':       {'path':'solar:','default':'wavelength', \
						'flg':'-w','type':str,'help':'name of input wavelength table column)'},
		'wavcol':     {'path':'solar:','default':'wavelength', \
						'flg':'-W','type':str,'help':'name of output wavelength table column)'},
		'wresolution':{'path':'solar:','default':None,'dshow':'(3pix*dispersion)', \
						'flg':'-r','type':float,'help':'wavelength resolution'},
	...
	logging.info ('Input number of pixels is {0}, mean dispersion is {1:.5f}'.format(len(w),mdisp))
	logging.info ('Mean input pixel resolution R_pix=lambda/dlambda is {0:.5f}'.format(mresol))
	...
	logging.info ('Output pixel resolution lambda/dlam: {0:.4f}'.format(np.mean(wave)/disp))
	logging.info ('Output spectral resolution: R={0:.2f}, dwave={1:.2f} nm'.format(R,info['wresolution']))

	# ---- CREATE DATA WITH OUTPUT RESOLUTION
	logging.info ('degrading reference spectrum to resolution {0:.3f} nm ...'.format(info['wresolution']))
	try :
