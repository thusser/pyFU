...
from pyFU.utils    import centroid, cubic, read_tables, line, vectorize, write_tables, Gaussian1D, get_infiles_and_outfiles
...
def cc_calibrate (wav,flx, refwav,refflx, show=True, spacing=100, width=100, model='linear', flux_order=None, flux_func='laguerre', wave_ranges=None) :
	"""
	...
	while n2 < nwav :
		...
		for wav_range in wave_ranges :
			if (wavg > wav_range[0]) and (wavg < wav_range[1]) : ok = True
...
def dispersion_fit (px,pxerr,w, model='linear', show=False, reversed=False) :
	...
	if reversed :
		x = np.array(px)
		y = np.array(w)
		err = x-x+1.e-4*np.mean(y)	# FAKE ERROR
		xname = 'pix'
		yname = 'wav'
		label = '# No.   Pixel     Wavelength  "Err"(Wave)  Fit(Wave)  Diff      Diff/Err'
		datafmt = '# {0:3d}  {1:8.3f}  {2:8.3f}    {3:7.3f}      {4:8.3f}   {5:8.3f}  {6:8.3f}'
	...
	if show :
		...
		dyfit = y-x2y(x,*coef)
		plt.plot ([x[0],x[-1]],[0.,0.],'-',color='green')
...
def cc_calibrate_spectra (spectra,refwav,refflx,
	...
	for idx in range(len(spectra)) :
		...
		# CALIBRATE
		px2wv,wv2px,flxcorr = cc_calibrate (wav,flux, refwav,refflx, \
				show=show, width=width, spacing=spacing, model=model, \
				flux_order=flux_order, flux_func=flux_func, wave_ranges=wave_ranges)
		...
		if flux_func is not None :
			...
			spectrum['flux_correction'] = np.zeros(len(pix))+1.
		chi2s.append(pchi2)
		rmss.append(prms)
		...
		for i in range(len(wcoef)) :
			hdr['WV2PXC{0:02d}'.format(i)] = (wcoef[i],'{0}-th coefficient of wave(pixel) function'.format(i))
...
def transfer_wavelengths_by_index (refs,extracted, pixcol='pixel', wavcol='wavelength', flxcol='flux') -> bool :
	...
	n = len(refs)
	if n != len(extracted) :
		logging.critical ('transfer_calibration: different number of spectra: nrefs={0}, nextracted={1}'.format(n,len(extracted)))
		return False
...
wavelength calibration so that the regions to be cross-correlated are reasonably well-defined.
	"""
	arguments = {
		'approx':{'path':'wavcal:', 'default':None, \
			'flg':'-a','type':list,'help':'rough wavelength calibration w0,d0,... where wav ~ w0+d0*pix+...'},
		'copy':{'path':None,'default':None, \
			'flg':'-c','type':str,'help':'copy reference wavelength and flux calibration metadata to file'},
		'fcol':{'path':'wavcal:','default':'flux', \
			'flg':'-f','type':str,'help':'name of flux table column in reference'},
		'flxcol':{'path':'wavcal:','default':'flux', \
			'flg':'-F','type':str, 'help':'name of output flux table column)'},
		'flux_function':{'path':'wavcal:','default':None, \
			'flg':'-X','type':str,'help':'function for flux-calibration; polynomial|legendre|laguerre|hermite'},
		'flux_order':{'path':'wavcal:','default':5, \
			'flg':'-M','type':str,'help':'polynomial order for C-C flux-correction'},
		'generic':{'path':None,'default':None, \
			'flg':'-G','type':str,'help':'YAML file for generic wavelength calibration info'},
		'infiles':{'path':'wavcal:','default':None, \
			'flg':'-i','type':str,'help':'pathname(s) of input FITS or ascii table(s)'},
		'in_format':{'path':'wavcal:','default':None, \
			'flg':'-I','type':str,'help':'optional format for finding spectrum in the pathname directory'},
		'outfiles':{'path':'wavcal:','default':None, \
			'flg':'-o','type':str, 'help':'output FITS table(s)'},
		'out_format':{'path':'wavcal:','default':None, \
			'flg':'-O','type':str,'help':'optional format for writing calibrated spectra to the pathname directory'},
		'model':{'path':'wavcal:','default':'linear', \
			'flg':'-m','type':str,'help':'model (linear|quadratic|cubic|exp|power'},
		'pause':{'path':None,'default':False, \
			'flg':'-P','type':bool,'help':'pause/prompt after every spectral calibration'},
		'pixcol':{'path':'wavcal:','default':'pixel', \
			'flg':'-x','type':str,'help':'name of pixel table column in target'},
		'plot':{'path':None,'default':False, \
			'flg':'-p','type':bool,'help':'plot result'},
		'reference':{'path':'wavcal:','default':None, \
			'flg':'-r','type':str,'help':'pathname of FITS reference spectrum'},
		'save':{'path':'wavcal:','default':None, \
			'flg':'-Y','type':int,'help':'YAML file for saving wavelength parameters'},
		'spacing':{'path':'wavcal:','default':120, \
			'flg':'-s','type':int,'help':'spacing of cross-correlation windows [pix]'},
		'wave_ranges':{'path':'wavcal:','default':None, 'dshow':'all', \
			'flg':'-R','type':list,'help':'wavelength ranges used l1,l2,l3,l4,...'},
		'wcol':{'path':'wavcal:','default':'wavelength', \
			'flg':'-w','type':str,'help':'name of wavelength table column in reference'},
		'wavcol':{'path':'wavcal:','default':'wavelength', \
			'flg':'-W','type':str,'help':'name of output wavelength table column'},
		'window_cc':{'path':'wavcal:','default':120, \
			'flg':'-N','type':int,'help':'size of cross-correlation windows [pix]'},
		'window_centroid':{'path':'wavcal:','default':11, \
			'flg':'-C','type':int,'help':'size of cross-correlation centroid window [pix]'},
	...
	if args.generic is not None :
		logging.info ('Appending generic wavelength calibration configuration info to'+args.generic)
	...
	for infile,outfile in zap(infiles,outfiles) :
		...
		if args.copy :
			logging.info (f'transfering wavelength calibration from reference to {infile}')
			if not transfer_wavelengths_by_index (reftab,spectra, wavcol=info['wavcol'], flxcol=info['flxcol']) :
				logging.error ('ABORTED spectral calibration via transfer from reference!')
				ok = False
		...
		# ---- OR PERFORM C-C CALIBRATION
			...
			if not cc_calibrate_spectra (spectra,refwav,refflx,
					...
					flux_func=info['flux_function'], wave_ranges=info['wave_ranges'],
					prompt=cfg['pause']) :
