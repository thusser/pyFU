...
class SpectrumExtractor (object) :
	...
	def __init__ (self, tracer, config=None, keywords=None, formats=None) :
		...
		# TRACE CONFIGURATION ...
		self.sigma = 3.		# DEFAULT JUST IN CASE
		self.number_traces = tracer.number_of_traces ()
		if self.number_traces <= 0 :
			logging.critical ('no traces available from tracer ({0})'.format(self.number_traces))
	...
	def set_column_names (self, pixcol='pixel', flxcol='flux', errcol='err_flux', labcol='label') :
		self.pixcol = pixcol
		self.flxcol = flxcol
		self.errcol = errcol
		self.labcol = labcol
	...
	def extract (self, hdu, dark=None, method='simple') -> dict :
		...
		if method not in ['simple','integrated'] :
			raise NotImplementedException ('{0} extraction method not implemented!'.format(method))
	...
	def _simple_extract (self, hdu, dark=None, integrate=False) -> list :
		...
		for idx in range(1,ns+1) :
			...
			if last_fibre is None or last_fibre.ampl_coef is None or last_fibre.sigma is None or last_fibre.trace_coef is None :
				last_sig        = self.sigma
			...
			if next_fibre is None or next_fibre.ampl_coef is None or next_fibre.sigma is None or next_fibre.trace_coef is None :
				next_sig        = self.sigma
			...
			erfs      =      sig*np.sqrt(2.)	# FUNNY NORMALIZATION OF erf()
			last_erfs = last_sig*np.sqrt(2.)
			next_erfs = next_sig*np.sqrt(2.)
			...
			for i in range(nx) :		# FOR ALL X-PIXELS
				...
				if j1 < 0 or j2 >= ny or n < 1 :
					...
					if not integrate :
						...
						last_prof = last_ac*np.exp(-(last_yc-yp)**2/last_sig**2)
						next_prof = next_ac*np.exp(-(next_yc-yp)**2/next_sig**2)

					# OR INTEGRATED GAUSSIAN WEIGHTING 0.5*(1-erf(dx/sqrt(2))), dx=(y-ycenter)/sigma
					else :
						prof      =      ac*0.5*(erf((yp-     yc+0.5)/erfs)     -erf((yp-     yc-0.5)/erfs))
						last_prof = last_ac*0.5*(erf((yp-last_yc+0.5)/last_erfs)-erf((yp-last_yc-0.5)/last_erfs))
						next_prof = next_ac*0.5*(erf((yp-next_yc+0.5)/next_erfs)-erf((yp-next_yc-0.5)/next_erfs))
			...
			# ADD METADATA
			if not self.filter.copy_header (hdu.header,t.meta,bare=True,comments=False) :
...
def main () :
	...
	arguments = {
		'errcol':{'path':'extract:','default':'err_flux', \
					'flg':'-E','type':str,'help':'name of flux error table column'},
		'vflip':{'path':'extract:','default':False, \
					'flg':'-V','type':bool,'help':'flip input image vertically before extracting'},
		'flxcol':{'path':'extract:','default':'flux', \
					'flg':'-F','type':str,'help':'name of flux table column'},
		'formats':{'path':None,'default':None, \
					'flg':'-f','type':str,'help':'YAML configuation file for FITS keyword formats'},
		'generic':{'path':None,'default':None, \
					'flg':'-G','type':str,'help':'YAML file for generic extraction configuration info'},
		...
		'trace':{'path':'trace:save','default':None, \
					'flg':'-T','type':str,'help':'path of (optional) YAML trace configuration file'},
		'yaml':{'path':None,'default':None, \
					'flg':'-y','type':str,'help':'path of (optional) generic YAML configuration file'}
		}
	...
	if args.generic is not None :
		logging.info ('Appending generic extraction configuration info to'+args.generic)
	...
	for infile,outfile in zip(infiles,outfiles) :
		...
		if args.trace is not None :				# FROM TRACE YAML
			tracer = SpectrumTracer (hdu,config=cfg,formats=formats,keywords=keywords,external_traces=args.trace)
		...
		extractor = SpectrumExtractor (tracer,config=cfg,keywords=keywords, formats=formats)
		...
		if args.plot :
			...
			plt.title (infile)
			ncol=1
			if len(spectra) > 10 : ncol=2
			plt.legend (bbox_to_anchor=(1.02,1), loc='upper left', ncol=ncol, fontsize='x-small')
			plt.tight_layout ()
			plt.show ()
