...
class Fibre(object):
    ...
    def _init_from_config(self, config):
        ...
        if config is None or len(config) == 0:
            logging.error(
                "cannot initialize fibre from a null/empty config dictionary!"
            )
            return
        info = config
        labels = None
    ...
    def update_header(self, header, mode="single") -> bool:
        ...
        if self.trace_coef is not None:
            ...
            if tfmt is None:
                logging.error(
                    "fibre #{0} does not have a trace coefficent format".format(
                        self.index
                    )
                )
                return False
        ...
        if self.ampl_coef is not None:
            ...
            if afmt is None:
                logging.error(
                    "fibre #{0} does not have an amplitude coefficent format".format(
                        self.index
                    )
                )
                return False
        ...
        for thing, fmtkey, key in things:
            if thing is not None:
                ...
                if fmtkey not in self.formats:
                    logging.warning(
                        "fibre #{0} has no header format for {1}".format(
                            self.index, fmtkey
                        )
                    )
    ...
    def get_trace_position_model(self):
        ...
        if self.trace_coef is None:
            raise IndexError(
                "no trace model for fibre #{0}/{1}".format(self.index, self.label)
            )
    ...
    def get_trace_amplitude_model(self):
        ...
        if self.ampl_coef is None:
            raise IndexError(
                "no amplitude coefficients for fibre #{0}/{1}".format(
                    self.index, self.label
                )
            )
        if self.ampl_model is None:
            raise IndexError(
                "no amplitude model for fibre #{0}/{1}".format(self.index, self.label)
            )
...
def hexpos(nfibres, diam):
    ...
    for i in range(khex):  # CENTRAL ROW
        x = xhex + diam * i
        positions[n] = [int(x * 100) / 100, 0.0]
        n += 1

    dx = 0.5 * diam
    dy = diam * np.sqrt(3.0 / 4.0)
...
def get_fibres(
    ...
    if header is not None:
        merge_dictionaries(
            cfg, header2config(header, keywords=keywords, formats=formats)
        )
