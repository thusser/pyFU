...
class SpectrumTracer(object):
    ...
    def _parse_info(self, ignore=False):
        ...
        if self.dy_max is None:
            self.dy_max = 1.5 * self.spacing
        logging.info(
            f"spacing={self.spacing},dy_min={self.dy_min},dy_max={self.dy_max}"
        )
    ...
    def use_external_slices(self, tab):
        ...
        for key in keys:
            if key not in names:
                raise ValueError(
                    "{0} not a column in external slice Table! Need keys {1}".format(
                        key, str(keys)
                    )
                )
    ...
    def find_maxima(self, show=False) -> bool:
        ...
        for slcpos in slice_pos:
            ...
            if peaks is not None and len(peaks) > 0:
                ...
                ywids = []
                logging.debug(
                    f"finding peak pos,avg,wid: window={self.window_centroid}"
                )
                for y in coords:
        ...
        for idx in range(0, nf):
            ...
            if d is not None:
                ...
                if showing:
                    ...
                    plt.plot(
                        xx, fa(xx, *pa), "-", label="{0} fit".format(self.ampl_model)
                    )
                    ...
                    plt.tight_layout()
                    reslt = show_with_menu(
                        fig, ["no more amplitude plots", "save to file", "ABORT"]
                    )
...
def main():
    ...
    arguments = {
        "ampl_model": {
            ...
            "flg": "-A",
            "type": str,
            "help": 'x model for traces ("polynomial"|"gaussians")',
        },
        "ampl_order": {
            "path": "trace:",
            "default": 5,
            "flg": "-a",
            "type": int,
            "help": "order of polynomial fit to spectra amplitudes (default 5)",
        },
        "bgk_factor": {
            "path": "trace:",
            "default": 1,
        ...
        "generic": {
            ...
            "flg": "-G",
            "type": str,
            "help": "YAML file for generic trace configuration info",
        },
        "sigma": {
            "path": "trace:",
            "default": 3,
        ...
        "ignore": {
            ...
            "flg": "-I",
            "type": bool,
            "help": "ignore trace info in FITS header (default False)",
        },
        "mid_slice": {
            "path": "trace:",
            "default": None,
        ...
        "plot": {
            ...
            "flg": "-p",
            "type": str,
            "help": "matplotlib backend (e.g. MacOSX|TkAgg,..) (default None)",
        },
        "sigma_order": {
            "path": "trace:",
            "default": 3,
            "flg": "-o",
            "type": int,
            "help": "polynomial order of fit to width variations",
        },
        "trace_order": {
            "path": "trace:",
            "default": 5,
