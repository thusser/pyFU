...
def cursor_input(data, show_method=show_hdu, *args, **kwargs):
    ...
    pts = []
    pt = plt.ginput(
        1, timeout=-1, show_clicks=True, mouse_add=1, mouse_pop=3, mouse_stop=2
    )
    while len(pt) == 1:
        pts.append(pt)
        pt = plt.ginput(
...
class ItemProperties(object):
    def __init__(self, fontsize=14, labelcolor="black", bgcolor="gray", alpha=1.0):
...
def show_with_menu(
    fig, items, on_select=show_menu_reaction, left=0.2, bottom=0.2, top=0.9, right=0.9
):
    """
    To test:
            fig = plt.figure ()
    ...
    hoverprops = ItemProperties(
        labelcolor="white", bgcolor="blue", fontsize=12, alpha=0.2
    )
    menuitems = []
    for label in items:
        item = SimpleMenuItem(
            fig, label, props=props, hoverprops=hoverprops, on_select=on_select
        )
...
def main():
    ...
    parser.add_argument(
        "--hdu", "-H", default=0, type=int, help="image HDU to display (default 0)"
    )
    parser.add_argument(
        "--images", "-i", default=None, help="path(s) of input FITS image(s)"
    )
    parser.add_argument(
        "--index", "-I", default=None, type=int, help="index of spectrum (1,..,number)"
    )
    parser.add_argument(
        "--labeled",
        "-l",
    ...
    parser.add_argument(
        "--outfiles", "-o", default=None, help="path(s) of output JPG,PNG,... image(s)"
    )
    parser.add_argument(
        "--xcol", "-X", default="wavelength", help="name of x-column in table"
    )
    parser.add_argument(
        "--ycol", "-Y", default="flux", help="name of y-column in table"
    )
    parser.add_argument(
        "--tables", "-t", default=None, help="path(s) of input binary FITS table(s)"
    )
    parser.add_argument(
        "--value",
        "-V",
    ...
    parser.add_argument(
        "--zmax", "-Z", type=float, default=None, help="upper displayed value"
    )
    parser.add_argument(
        "--zmin", "-z", type=float, default=None, help="lower displayed value"
    )
    parser.add_argument(
        "--zrange", "-R", default=None, help="lower,upper displayed values"
    )
